...
> 更多配置排查命令（配置来源诊断、生成默认配置等）见 [配置系统 · 故障诊断](./configuration.md#故障诊断)。
...
> **传输层环境变量**：`NEGENTROPY_PERCEIVES_TRANSPORT_MODE`（默认 `http`）、`_HTTP_HOST`、`_HTTP_PORT`、`_HTTP_PATH`、`_HTTP_CORS_ORIGINS`。完整默认值与约束见 [配置系统 · 传输层](./configuration.md#传输层)。
...
> 也可通过用户 YAML 配置文件 `~/.negentropy/perceives.config.yaml` 进行持久化配置，模板与示例见 [配置系统 · 环境配置模板](./configuration.md#环境配置模板)。
...
以下配置通过环境变量 `NEGENTROPY_PERCEIVES_*` 或用户 YAML 文件管理，完整参数见 [配置系统](./configuration.md)：

| 配置类别 | 典型变量 | 说明 |
|----------|---------|------|
| [代理服务](./configuration.md#代理服务) | `USE_PROXY`、`PROXY_URL` | 通过代理服务器发起请求 |
| [缓存系统](./configuration.md#缓存系统) | `ENABLE_CACHING`、`CACHE_TTL_HOURS` | 响应缓存与 TTL 控制 |
| [硬件加速](./configuration.md#硬件加速) | `ACCELERATOR_DEVICE`、`ACCELERATOR_NUM_THREADS` | GPU/MPS/CPU 推理设备选择 |
| [LLM 编排](./configuration.md#llm-编排smart-模式) | `LLM_API_KEY`、`LLM_MODEL` | Smart 模式多引擎 PDF 处理 |
| [速率限制](./configuration.md#速率限制) | `RATE_LIMIT_REQUESTS_PER_MINUTE` | 请求频率控制 |
| [重试策略](./configuration.md#重试策略) | `MAX_RETRIES`、`RETRY_DELAY` | 失败重试机制 |
...
> 代理配置详情见 [配置系统 · 代理服务](./configuration.md#代理服务)。
...
> 缓存系统详情见 [配置系统 · 缓存系统](./configuration.md#缓存系统)。
...
> 设备检测优先级、batch size 自动推断规则及平台特定行为见 [配置系统 · 硬件加速](./configuration.md#硬件加速)。
