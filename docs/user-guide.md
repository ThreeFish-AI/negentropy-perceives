...
- **6 个专业 MCP 工具**：链接提取、页面信息、网页转 Markdown、PDF 转 Markdown（含批量版本）
...
服务默认以 HTTP 模式启动在 `http://localhost:8081/mcp`。
...
| 工具名称                                | 模块              | 功能                     | 返回类型               |
| --------------------------------------- | ----------------- | ------------------------ | ---------------------- |
| `extract_links`                         | `extraction.py`   | 提取网页链接 + 域名过滤  | `LinksResponse`        |
| `get_page_info`                         | `extraction.py`   | 获取页面元数据           | `PageInfoResponse`     |
| `convert_webpage_to_markdown`           | `markdown.py`     | 网页 → Markdown          | `MarkdownResponse`     |
| `batch_convert_webpages_to_markdown`    | `markdown.py`     | 批量网页 → Markdown      | `BatchMarkdownResponse`|
| `convert_pdf_to_markdown`               | `pdf.py`          | PDF → Markdown           | `PDFResponse`          |
| `batch_convert_pdfs_to_markdown`        | `pdf.py`          | 批量 PDF → Markdown      | `BatchPDFResponse`     |
...
### 3. convert_webpage_to_markdown — 网页转 Markdown
...
### 4. batch_convert_webpages_to_markdown — 批量网页转 Markdown
...
### 5. convert_pdf_to_markdown — PDF 转 Markdown
...
> **引擎门控**：`docling_enabled`、`mineru_enabled`、`marker_enabled` 三个环境变量控制各引擎的可用性。未安装对应依赖的引擎会自动跳过（`is_available()` 检测），不会报错。

### 6. batch_convert_pdfs_to_markdown — 批量 PDF 转 Markdown
...
# 基础连接（默认 http://localhost:8081/mcp）
...
async with NegentropyPerceivesClient(
    ...
    result = await client.call_tool("get_page_info", {"url": "https://example.com"})
```
...
md_result = converter.convert_webpage_to_markdown(
...
# 客户端带超时调用
result = await client.call_tool("convert_webpage_to_markdown", {"url": "..."}, timeout=30.0)
```
