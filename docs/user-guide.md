...
服务默认以 HTTP 模式启动在 `http://localhost:8092/mcp`。
...
async with NegentropyPerceivesClient("http://localhost:8092/mcp") as client:
    # 网页 → Markdown
...
| `NEGENTROPY_PERCEIVES_HTTP_PORT`         | `int`  | `8092`      | HTTP 服务器端口                              |
...
NEGENTROPY_PERCEIVES_HTTP_PORT=8092 \
negentropy-perceives
```
...
# 基础连接（默认 http://localhost:8092/mcp）
