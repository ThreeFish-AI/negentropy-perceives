...
> **引擎门控**：`docling_enabled`、`mineru_enabled`、`marker_enabled` 三个环境变量控制各引擎的可用性（默认均为 `true`）。未安装对应依赖的引擎会自动跳过（`is_available()` 检测），不会报错；如需在已安装环境下显式禁用某引擎，可通过环境变量或 YAML 覆盖为 `false`。

#### 启用四大 PDF 引擎（Docling / MinerU / Marker / PyMuPDF）

默认仅 `PyMuPDF` 随核心包一并安装。若希望 `layout_analysis` / `table_extraction` / `formula_extraction` 等竞争模式 Stage 真正利用 Docling、MinerU、Marker 四大引擎同台竞技，请参照 [开发指南 · 详细环境配置](development.md#详细环境配置) 通过 `all-engines` 或单独 extras 安装可选依赖。
...
结合每个 Stage 启动时的 `Stage 'layout_analysis' 参与竞争 tools=[...]` 日志，即可快速确认本次实际参与竞争的工具集合。

> **预热模型（推荐）**：安装完引擎后，首次请求会现场下载 ~1.35GB 的 Marker
> Layout 模型，极易触发 120s Stage 超时。请先执行 `uv run perceives prefetch-models`
