...
默认仅 `PyMuPDF` 随核心包一并安装。要让 `layout_analysis` / `table_extraction` / `formula_extraction` / `code_detection` 等 Stage 充分发挥能力（默认已切换为单引擎降级模式，rank=1 默认为 Docling/MinerU），请参照 [开发指南 · 详细环境配置](development.md#详细环境配置) 通过 `all-engines` 或单独 extras 安装可选依赖。
